...
export default class DynamicOptionList extends React.Component {
  ...
  editValue(option_index, e) {
    const this_element = this.state.element;
    const val = (e.target.value === '') ? this._setValue(this_element.TypeDetail[option_index].text) : e.target.value;
    this_element.TypeDetail[option_index].value = val;
    this.setState({
      element: this_element,
      dirty: true,
        ...
        <ul>
          ...
          </li>
          {
            Object.keys(obj).map((option) => {
              ...
              const this_key = `edit_${obj[option]}`;
              const val = (obj[option] !== this._setValue(obj[option])) ? obj[option] : '';
              return (
                <li className="clearfix" key={this_key}>
                  <div className="row">
                  <div className="col-sm-6">
            <input tabIndex={option} className="form-control" style={{ width: '100%' }} type="text" name={`text_${obj[option]}`} placeholder="Option text" value={obj[option]} onBlur={this.updateOption.bind(this)} onChange={this.editOption.bind(this, option)} />
          </div>
                 ...
                 <button onClick={this.addOption.bind(this, option)} className="btn btn-success"><i className="fas fa-plus-circle"></i></button>
              { obj 
                && <button onClick={this.removeOption.bind(this, option)} className="btn btn-danger"><i className="fas fa-minus-circle"></i></button>
              }
